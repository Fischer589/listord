...
type DepositStatus = "checking" | "paid" | "pending" | "failed" | "unknown";

export default function EmployerDepositSuccessPage({
  searchParams
}: {
  ...
  useEffect(() => {
    ...
    // The frontend NEVER marks the deposit as paid — it only polls the
    ...
    async function poll() {
      ...
      try {
        const response = await fetch(
          `/api/stripe/employer-deposit-status?session_id=${encodeURIComponent(sessionId!)}`
        );
      ...
      <div className="container" style={{ maxWidth: "520px", textAlign: "center", padding: "3rem 1.5rem" }}>
        ...
        {status === "paid" && (
          <>
            ...
            <p style={{ color: "rgba(26,61,43,0.65)", fontSize: "1.05rem", marginBottom: "2rem", lineHeight: 1.6 }}>
              ListoRD comenzará el proceso de búsqueda de personal.
            </p>
          </>
        )}
        ...
        {(status === "checking" || status === "pending") && (
          <>
            ...
            <p style={{ color: "rgba(26,61,43,0.65)", fontSize: "1.05rem", marginBottom: "2rem", lineHeight: 1.6 }}>
              Esto normalmente toma unos segundos. Si tu pago fue exitoso, lo confirmaremos automáticamente.
            </p>
          </>
        )}
        ...
        {(status === "failed" || status === "unknown") && (
          <>
            ...
            <p style={{ color: "rgba(26,61,43,0.65)", fontSize: "1.05rem", marginBottom: "2rem", lineHeight: 1.6 }}>
              Si Stripe confirmó tu pago, lo registraremos en breve. Si tienes dudas, escríbenos por WhatsApp.
            </p>
          </>
        )}
